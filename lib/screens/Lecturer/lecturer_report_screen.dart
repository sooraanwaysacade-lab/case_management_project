...
class LecturerReportScreen extends StatefulWidget {
  const LecturerReportScreen({super.key});

  @override
  State<LecturerReportScreen> createState() => _LecturerReportScreenState();
}

class _LecturerReportScreenState extends State<LecturerReportScreen> {
  ...
  Future<void> _submitReport() async {
    ...
    try {
      final reporterName = user?.displayName ?? user?.email?.split('@').first ?? "Unknown";
      final reporterEmail = user?.email ?? "unknown@umma.ac.ke";
      ...
      final caseTitleText = caseTitle.text.trim();
      final studentIdText = studentId.text.trim();
      final studentEmailText = studentEmail.text.trim();
      ...
      await FirebaseFirestore.instance.collection('notifications').add({
        ...
        'body':
            'A new case titled "$caseTitleText" with Student ID $studentIdText was reported by $reporterEmail.',
        'caseId': newCase.id,
        'type': 'case_created',
        'createdAt': FieldValue.serverTimestamp(),
        'isRead': false,
      ...
      // Notify student (reported user)
      if (studentEmailText.isNotEmpty) {
        await FirebaseFirestore.instance.collection('notifications').add({
          'targetEmail': studentEmailText,
          'title': 'You Have Been Reported',
          'body':
              'You have been reported in the case "$caseTitleText" by $reporterEmail. Please check your case for details.',
          'caseId': newCase.id,
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            Icon(Icons.email, color: Colors.indigo.shade600, size: 20),
                            const SizedBox(width: 8),
                            Text(
                              'Email: ${user?.email ?? "unknown@umma.ac.ke"}',
                              style: TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.w500,
                                color: Theme.of(context).textTheme.bodyLarge?.color ?? Colors.black87,
