...
import 'lecturer_case_details_screen.dart';

class LecturerAssignedCasesScreen extends StatefulWidget {
  const LecturerAssignedCasesScreen({super.key});

  @override
  State<LecturerAssignedCasesScreen> createState() =>
      _LecturerAssignedCasesScreenState();
}

class _LecturerAssignedCasesScreenState
    extends State<LecturerAssignedCasesScreen> {
  ...
  Future<void> _loadLecturerEmail() async {
    ...
    setState(() {
      lecturerEmail = currentUser?.email?.toLowerCase() ?? '';
    });
  }

  /// 🔍 FIRESTORE QUERY — Lecturer's assigned cases
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snap) {
                ...
                var docs = List<DocumentSnapshot>.from(snap.data!.docs);

                // Sort by caseTitle (client-side to avoid composite index requirement)
                docs.sort((a, b) {
                  final aTitle = (a.data() as Map<String, dynamic>)['caseTitle'] ?? '';
                  final bTitle = (b.data() as Map<String, dynamic>)['caseTitle'] ?? '';
                    ...
                    return _buildCaseCard(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => LecturerCaseDetailsScreen(caseDoc: docs[i]),
                          ),
                        );
                      },
                    );
